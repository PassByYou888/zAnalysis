...
- 注意：
		sift高斯金字塔不支持分色尺度和分色差分计算，内核是把彩色转换成灰度后进行的尺度和差分计算
		sift高斯金字塔不支持单应性矩阵
		sift高斯金字塔不支持模式识别，需要手动判断场景模式
		对500亿以上的平方运算仍然使用的传统浮点+并行化，未使用sse2,avx512,gpu等新技术接口，图片过于庞大时将会非常耗时
		本次更新未编写任何使用文档，具体使用zAnalysis请参考Demo范式以及阅读开源代码
