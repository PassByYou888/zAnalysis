...
type
  ...
  public
    ...
    procedure DrawUserCustom(const UserProc: TUserCustomDrawProc; const UserData: Pointer; const UserObject: TCoreClassObject); {$IFDEF INLINE_ASM}inline; {$ENDIF}
    { }
    procedure DrawPLInScene(pl: T2DPointList; ClosedLine: Boolean; opt: TPolyDrawOption); {$IFDEF INLINE_ASM}inline; {$ENDIF}
...
begin
  try
    case t of
      dctSetSize:
        with PDrawCommandParam_1Rect(data)^ do
            IDraw.SetSize(r);
      dctSetLineWidth:
        with PDrawCommandParam_1Float(data)^ do
            IDraw.SetLineWidth(f);
      dctLine:
        with PDrawCommandParam_pt_Color(data)^ do
            IDraw.DrawLine(pt1, pt2, color);
      dctDrawRect:
        with PDrawCommandParam_Rect_Color(data)^ do
      ...
      dctDrawText:
        with PDrawCommandParam_Textout(data)^ do
            IDraw.DrawText(text, size, r, color, Center);
      dctDrawTexture:
        with PDrawCommandParam_Texture(data)^ do
            IDraw.DrawTexture(t, sour, dest, alpha);
      dctUserCustom:
        with PDrawCommandParam_UserCustom(data)^ do
...
begin
  if ReadyOK then
    begin
      ...
      RaiseInfo('interface no readyOk');
end;
...
procedure TDrawEngine.DrawPLInScene(pl: T2DPointList; ClosedLine: Boolean; opt: TPolyDrawOption);
var
  i: Integer;
  t1, t2: TDEVec;
  r: TDERect;
    ...
    begin
      ...
      if p^.workObj = workObj then
        begin
          ...
          if p^.DrawEng.DrawInterface <> ADrawInterface then
              p^.DrawEng.DrawInterface := ADrawInterface;
