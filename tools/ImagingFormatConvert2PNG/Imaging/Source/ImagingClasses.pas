...
begin
  if Source = nil then
  begin
    Self:=TSingleImage.Create;
  end
  else if Source is TSingleImage then
  begin
    Self:=TSingleImage.CreateFromData(TSingleImage(Source).FImageData);
  end
  else if Source is TMultiImage then
  begin
    if TMultiImage(Source).Valid then
      Self:=TSingleImage.CreateFromData(TMultiImage(Source).FPData^)
    else
      Assign(nil);
  end
  else
...
begin
  if Source = nil then
  begin
    Self:=TMultiImage.Create;
  end
  else if Source is TMultiImage then
  begin
    Self:=TMultiImage.CreateFromArray(TMultiImage(Source).FDataArray);
  ...
  begin
    ...
    Self:=TMultiImage.CreateFromArray(Arr);
    Arr := nil;
  end
  else
    inherited Assign(Source);
